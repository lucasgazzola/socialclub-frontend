...
export function ComprarEntradasPage() {
   ...
   */
  const { mutateAsync, isPending, error: errorCrear } = useCrearEntradas(); // Hook-Mutacion para crear entradas
   ...
   */
  const [entradasGeneradas, setEntradasGeneradas] = useState<Entrada[]>([]);
   ...
   */
  const maxCantidad = evento?.entradasDisponibles ?? 1; 
  
  /**
   * Ejecuta el Hook y devuelve las entradas generadas por el backend.
   * Tambien actualiza el estado de entradasGeneradas y eventoNombre para renderizar los QR.
   */
  const handleGenerar = async () => {
    const resultado = await mutateAsync({ eventoId: id, cantidad });
    setEntradasGeneradas(resultado.entradas);
    setEventoNombre(resultado.eventoNombre);
  };

  /** 
   * Genera un QR para la entrada y lo descarga como imagen PNG.
  */
  const descargarQR = (entrada: Entrada) => {
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-center gap-3">
        <Button variant="ghost" size="sm" onClick={() => navigate(ROUTES.eventos)}>
          <ArrowLeft size={24} />
        </Button>
        <div>
          ...
          {entradasGeneradas.length > 0 && (
            <div>
              <h2 className="mb-3 text-lg font-semibold text-slate-900">
                {entradasGeneradas.length} entrada(s) generada(s) para {eventoNombre}
              </h2>
