...
describe('UserForm', () => {
  describe('Modo creación', () => {
    it('permite completar los datos básicos y enviar un usuario válido', async () => {
      ...
      render(<UserForm modo="crear" onSubmit={onSubmit} />);
    ...
    it('muestra el rol por defecto ADMIN en modo creación', () => {
      render(<UserForm modo="crear" onSubmit={vi.fn().mockResolvedValue(undefined)} />);
  ...
  describe('Modo edición', () => {
    ...
    it('precarga los datos del usuario existente en el formulario', () => {
      render(
        <UserForm modo="editar" usuarioInicial={usuarioInicial} onSubmit={vi.fn().mockResolvedValue(undefined)} />,
      );
    ...
    it('permite cambiar el rol del usuario en edición', async () => {
      ...
      const colaboradorCheckbox = screen.getByRole('checkbox', { name: /collaborator/i });
      ...
      expect(onSubmit).toHaveBeenCalledWith(
        expect.objectContaining({
          roles: expect.arrayContaining(['ADMIN', 'COLLABORATOR']),
        }),
        expect.anything(),
      );
    });
